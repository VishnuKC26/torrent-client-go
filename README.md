...
This project is inspired by and structurally aligned with the [torrent-client](https://github.com/veggiedefender/torrent-client) by veggiedefender and the [BitTorrent protocol walkthrough](https://blog.jse.li/posts/torrent/) by Jesse Li.
...
1. **Parse**: Decodes the `.torrent` file and extracts the cryptographic info-hash.
2. **Discover**: Contacts the tracker to receive a list of active peers in the swarm.
3. **Handshake**: Establishes a TCP connection and verifies the protocol handshake with peers.
4. **Download**: Workers request blocks, assemble them into pieces, and verify them against the info-hash.
5. **Assemble**: Once all pieces are verified, they are joined to create the final output file.
...
> [!IMPORTANT]  
> Because BitTorrent relies on a "tit-for-tat" mechanism, some peers in public swarms (like Debian) may refuse to unchoke this client because it is not uploading back to them. This is expected behavior for a download-only educational client.
...
- [Jesse Li's BitTorrent Walkthrough](https://blog.jse.li/posts/torrent/)
